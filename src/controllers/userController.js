...
const login = async function (req, res){
    ...
    let foundUser = await UserModel.findOne({emailId : userName, password : password})

    if(!foundUser) return res.send({status: false, msg: "Incorrect username or password"})
   
    let token = jwt.sign({userId : (foundUser._id).toString()}, "surajdubey")
