  ...
  try {
    ...
    if (!isValidRequestBody(requestBody)) {
      return res.status(400).send({
        status: false,
        message: "Book data is required to create a new user",
      });
    }
    ...
    if (!isValid(title)) {
      return res.status(400).send({
        status: false,
        message: `title is required and should be in valid format like: "Atomic habits"`,
      });
    }
    ...
    if (!isValid(excerpt)) {
      return res.status(400).send({
        status: false,
        message: `excerpt is required and should be in valid format`,
      });
    }
    ...
    if (!isValid(userId)) {
      return res.status(400).send({
        status: false,
        message: `userId is required and should be in valid format`,
      });
    }
    ...
    if (!isValid(category)) {
      return res.status(400).send({
        status: false,
        message: `category is required and should be in valid format like: Fiction`,
      });
    }
    ...
    if (Array.isArray(subcategory)) {
      for (let i = 0; i < subcategory.length; i++) {
        ...
        if (!isValid(element)) {
          return res.status(400).send({
            status: false,
            message: `subcategory is required and should be in valid format like: [Romantic, cool] `,
          });
        }
      }
    }
    ...
    if (!isValidSubcategory(subcategory)) {
      return res.status(400).send({
        status: false,
        message: `subcategory is required and should be in valid format like: Romantic `,
      });
    }
    ...
    if (isValidRequestBody(queryParams)) {
      ...
      if (queryParams.hasOwnProperty("subcategory")) {
        if (isValidSubcategory(subcategory)) {
          ...
          return res.status(400).send({
            status: false,
            message: `enter subcategory in valid format`,
          });
        }
      }
  ...
  try {
    ...
    if (requestBody.hasOwnProperty("title")) {
      if (isValid(title)) {
        ...
        if (isTitleUnique) {
          return res.status(400).send({
            status: false,
            message: " Book title already exist. It should be unique ",
          });
        }
        ...
        return res.status(400).send({
          status: false,
          message: `enter book title in valid format like : Atomic habits`,
        });
      }
    }
    ...
    if (requestBody.hasOwnProperty("excerpt")) {
      if (isValid(excerpt)) {
        ...
        return res.status(400).send({
          status: false,
          message: "enter book excerpt in valid format",
        });
      }
    }
    ...
    if (requestBody.hasOwnProperty("releasedAt")) {
      if (isValid(releasedAt)) {
        ...
        return res.status(400).send({
          status: false,
          message: "enter released date in valid format YYYY-MM-DD",
        });
      }
    }
    ...
    if (requestBody.hasOwnProperty("ISBN")) {
      if (isValid(ISBN)) {
        if (!/^(97(8|9))?\d{9}(\d|X)$/.test(ISBN.split("-").join(""))) {
          return res.status(400).send({
            status: false,
            message: `enter a valid ISBN of 13 digits`,
          });
        }
...
const deleteBook = async function (req, res) {
  try {
    ...
    if (isValidRequestBody(requestBody)) {
      return res.status(400).send({
        status: false,
        message: "input data is not required in request body",
      });
    }
